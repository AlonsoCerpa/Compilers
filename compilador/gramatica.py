...
class Gramatica:
    ...
    def cargar(self, texto):
        ...
        self.no_terminales = sorted(self.no_terminales)
        ...
        self.terminales = sorted(self.terminales)
        ...
        for idx, cuerpo in enumerate(cuerpos_produccion):
            if cuerpo[0] in self.terminales:
                if cuerpo[0] == "lambda":
                    ...
                    for prim in siguientes[no_term]:
    ...
    def analizar_cadena(self, entrada, tabla_sint, tokens):
        pila = deque()
        pila.append("$")
        ...
        while len(entrada) != 0 and len(pila) != 0:
            ...
            #print("pila", list(reversed(pila)))
            #print()
            if entrada[0] == pila[-1]:
                entrada.popleft()
                pila.pop()
                if (idx_token < len(tokens)):
                    nodo_actual.value = tokens[idx_token].palabra
                    nodo_actual.pos = tokens[idx_token].indice
                    idx_token += 1
